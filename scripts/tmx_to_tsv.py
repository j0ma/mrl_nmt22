...
# NOTE: originally sourced from mtdata:
#   https://github.com/thammegowda/mtdata/blob/master/mtdata/tmx.py
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(
        description="A tool to convert TMX to TSV",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    p.add_argument("-i", "--inp", type=Path, required=True, help="Input file path")
    p.add_argument(
        "-o", "--out", type=Path, default=Path("/dev/stdout"), help="Output file path"
    )
    p.add_argument(
        "-l",
        "--langs",
